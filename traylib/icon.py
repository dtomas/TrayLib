...
class Icon(gtk.EventBox, object):
    ...
    def __init__(self, config):
        ...
        @param config: The L{IconConfig} controlling the configuration of this
            C{Icon}.
        """
        assert isinstance(config, IconConfig)
        ...
        assert self.__config == config
        assert self.__config.has_configurable(self)
        assert self.__image
        assert self.__pixbuf == None
        assert self.__pixbuf_current == None
        assert self.__zoom_factor == 1.0
        assert self.__canvas == None
        assert self.__has_arrow == False
        assert self.__menu == None
        assert self.__mouse_over == False
        assert self.__blink_event == 0
        assert self.__target_size == self.__config.size
        assert self.__current_size == 1
        assert self.__zoom_event == 0
        assert self.__zoom_action == ZOOM_ACTION_NONE
        assert self.__arrow_target_alpha == 0
        assert self.__arrow_current_alpha == 0
        assert self.__emblem_orig == None
        assert self.__emblem_scaled == None
        assert self.__emblem_target_alpha == 0
        assert self.__emblem_current_alpha == 0
        assert self.__spring_open_event == 0
        assert self.__is_drop_target == False
        assert self.__is_dragged == False
        assert self.__has_themed_icon == False
        assert self.__icon_theme_changed_handler
